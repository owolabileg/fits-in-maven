...
before (): call(* *.*(..)) &&
   ...
   !cflowbelow(adviceexecution())  {
    if (Verification.initialised) {
      if (!Verification.triggered535e394e) {
        Verification.currentRegExp535e394e = Verification.currentRegExp535e394e.derivative(new Event("before", "#"));
        Verification.triggered535e394e = true;
      }
      if (Verification.currentRegExp535e394e.hasEmpty()) {
      ...
      Verification.triggered535e394e = false;
      if (!Verification.triggered4479ef22) {
        Verification.currentRegExp4479ef22 = Verification.currentRegExp4479ef22.derivative(new Event("before", "#"));
        Verification.triggered4479ef22 = true;
      }
      if (Verification.currentRegExp4479ef22.cannotMatch()) {
        Assertion.alert("Violation detected on  property { GOOD (((((!(before UserInfo.makeDisabled))* ; before UserInfo.makeDisabled) ; (!(before UserInfo.withdrawFrom))*) ; before UserInfo.makeEnabled))*}");
      }
      Verification.triggered4479ef22 = false;
      if (!Verification.triggered2a6a178c) {
        Verification.currentRegExp2a6a178c = Verification.currentRegExp2a6a178c.derivative(new Event("before", "#"));
        Verification.triggered2a6a178c = true;
      }
      if (Verification.currentRegExp2a6a178c.hasEmpty()) {
        Assertion.alert("Violation detected on  property { BAD ((((?)* ; before makeDisabled) ; (!(before makeEnabled))*) ; before withdrawFrom)}");
      }
      Verification.triggered2a6a178c = false;
      if (!Verification.triggered41d1dc80) {
        Verification.currentRegExp41d1dc80 = Verification.currentRegExp41d1dc80.derivative(new Event("before", "#"));
        Verification.triggered41d1dc80 = true;
      }
      if (Verification.currentRegExp41d1dc80.hasEmpty()) {
        Assertion.alert("Violation detected on  property { BAD ((((!(before UserSession.openSession))* ; (((before UserSession.openSession ; (!(before UserSession.closeSession))*) ; before UserSession.closeSession))*) ; (!(before UserSession.openSession))*) ; before UserSession.log)}");
      }
      Verification.triggered41d1dc80 = false;
      if (!Verification.triggered636719bd) {
        Verification.currentRegExp636719bd = Verification.currentRegExp636719bd.derivative(new Event("before", "#"));
        Verification.triggered636719bd = true;
      }
      if (Verification.currentRegExp636719bd.cannotMatch()) {
        Assertion.alert("Violation detected on  property { GOOD (((((!(before UserSession.log))* ; before UserSession.openSession) ; (!(before UserSession.closeSession))*) ; before UserSession.closeSession))*}");
      }
      Verification.triggered636719bd = false;
      }
  }
